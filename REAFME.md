...
```
uie可以一次性识别 出所有的实体和关系，不需要指定识别的实体类型和关系类型(这里是把所有的实体和关系都识别了)
一条数据识别所有实体即关系在cpu上识别时间 5.08s/it，这的时间慢主要与cpu有关系，同时与识别的实体个数有一定的关系，但感觉关系不太大 
```

### 优点2 缓解暴露偏差

```
暴露偏差是指训练时每个输入都来自于真实样本的标签，测试时输入却是来自上一个时刻的输出
生成负样本的过程
	在训练阶段加入没有标注的实体和关系，即负样本
	例如：真实的训练标签：<spot> 人物 <spot> + text
	加入负样本实体(出生日期)的训练数据：<spot> 人物 <spot> 出生日期<spot> + text
	在负样本的基础上加入关系负样本"学校",即训练数据：<spot> 人物 <spot> 出生日期<spot><asoc>学校<asoc> + text
	record(解码的输入)也要相应的加入相应的负样本
	
加入这个负样本的意义：
	1.给定识别的实体类型也不一定能识别出结果，这个实体是否要识别让模型去学。
	2.在解码阶段在record加入负样本缓解了上一个时刻的输出不准确的情况，即解码加入了负样本让模型去纠正这个负样本，提高泛化性
```
