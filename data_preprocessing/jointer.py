...
from data_preprocessing.utils import tokens_to_str, change_ptb_token_back
from data_preprocessing.ie_format import Entity, Label, Relation, Sentence, Span
from data_preprocessing.task_format import TaskFormat
...
class JointER(TaskFormat):
    ...
    def __init__(self, sentence_json, language='en'):
        ...
        self.spo_details = sentence_json['spo_details']  # 实体的详细信息(位置信息)
            ...
            if (s_s, s_e, s_t) not in entity_map:
                entities[(s_s, s_e, s_t)] = Entity(
                    span=Span(
                        tokens=tokens,
                        indexes=indexes,
                        text=tokens_to_str(tokens, language=self.language),  # join字符串
                    ),
                    label=Label(s_t)
                )
    ...
    def load_from_file(filename, language='en') -> List[Sentence]:
        sentence_list = list()
        raw_instance_list = json.load(open(filename, "r", encoding="utf-8"))
